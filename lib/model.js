...
function DynamoModel (tableName, schema, options) {
...
DynamoModel.prototype.getItem = function (key, options, callback) {
    ...
    this.waitForActiveTable(function (err) {
        ...
        return this.dynamodb.getItem(params, function (err, response) {
            if (err) return callback(err);
            return callback(null, this.schema.mapFromDb(response.Item), response);
        });
    });
};
...
DynamoModel.prototype.batchGetItems = function (keys, options, callback) {
    ...
    var params = options || {};
    params.RequestItems = {};
    params.RequestItems[this.tableName] = { Keys: keys.map( function(item) { this.schema.mapToDb(item); } ) };

    if (this.consistentRead) params.ConsistentRead = true;

    this.waitForActiveTable(function (err) {
        if (err) return callback(err);
        return this.dynamodb.batchGetItem(params, function (err, response) {
            if (err) return callback(err);

            if (data.Responses[this.tableName].length == 0) return callback(null, null, response);

            var results = data.Responses[this.tableName].map( function(item) {
              return this.schema.mapFromDb(item);
            });
...
DynamoModel.prototype.putItem = function (item, options, callback) {
    ...
    this.waitForActiveTable(function (err) {
        if (err) return callback(err);
        this.dynamodb.putItem(params, function (err, response) {
            if (err) return callback(err);
            callback(null, this.schema.mapFromDb(response.Attributes), response);
        });
    });
};
                ...
                instance.describeTable(function (err, response) {
                    if (err) {
                        if (err.code === 'ResourceNotFoundException') {
                            ...
                            createTable(function (err, response) {
                                if (err) return callback(err);
