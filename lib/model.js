...
DynamoModel.prototype.batchGetItems = function (keys, options, callback) {
    ...
    var params = options || {};
    params.RequestItems = {};
    params.RequestItems[this.tableName] = { Keys: keys.map( function(item) { instance.schema.mapToDb(item); } ) };
    ...
    this.waitForActiveTable(function (err) {
        if (err) return callback(err);
        return instance.dynamodb.batchGetItem(params, function (err, response) {
            if (err) return callback(err);
