...
function DynamoModel (tableName, schema, dynamodb, options) {
    ...
    this.consistentRead = this.options.ConsistentRead ? this.options.ConsistentRead : false;
    this.defaultThroughput = {
        ReadCapacityUnits: this.options.ReadCapacityUnits ? this.options.ReadCapacityUnits : 10,
        WriteCapacityUnits: this.options.WriteCapacityUnits ? this.options.WriteCapacityUnits : 5
    };

    // make sure the table is available as soon as possible
    if (this.options.skipWaitForActiveTable === true) return;
