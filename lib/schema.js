function DynamoSchema (schema, instanceFactory) {

    this.schema = schema;

    this.instanceFactory = function() { return {}; }
    if (instanceFactory) this.instanceFactory = instanceFactory;
...
DynamoSchema.prototype.mapFromDb = function(doc) {
    if (!doc) return null;
    var mappedDoc = this.instanceFactory();
...
DynamoSchema.mappers = {
    ...
    'Date': {
        dynamoDbType: 'N',
        mapFromDb: function (value) {
            return value.N === 'null' ? null : new Date(Number(value.N));
        },
        mapToDb: function (value) {
            var toReturn = value ? value.getTime() : null;
            return { N: String(toReturn) };
        }
    },
    'JSON': {
        dynamoDbType: 'S',
